...
async def locktypes_cmd(client, message: Message, _):
    ...
    # FIX: show 'all' on top, keep rest same
    lines = [f"{EMOJI.get('all','')} all"] + [f"{EMOJI.get(t,'')} {t}" for t in LOCKABLES if t != "all"]
...
# ------------------------------
# WATCHER - DELETE LOCKED MESSAGES (FINAL FIX)
# ------------------------------

@app.on_message(filters.group, group=69)  # increased priority to avoid being skipped
async def lock_watcher(client, message: Message):
    try:
        ...
        # Admin + LockAdmin: if lockadmin enabled, restrict admins too for media
        ...
        # Normal users (FULL FIX)
        elif not is_admin:
            # If "all" is enabled — delete any kind of user content including text/caption
            if data.get("all"):
                if (
                    message.text
                    or message.caption
